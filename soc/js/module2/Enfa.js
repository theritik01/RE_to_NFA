...
export default class Enfa {
  ...
  add_transition(from, via, to) {
    ...
    } else if (this.transition[from][via] == undefined)
      this.transition[from][via] = [to];
  ...
  concat(obj) {
    ...
    this.final_states.forEach((s) => {
      this.add_transition(
        s,
        sigma,
        this.get_state_val_with_offset(obj.intial_state, offset)
      );
    });

    this.final_states = [];
    obj.final_states.forEach((el) =>
      this.final_states.push(this.get_state_val_with_offset(el, offset))
    );
  ...
  addition(obj) {
    ...
    result.add_transition(
      result.intial_state,
      sigma,
      this.get_state_val_with_offset(this.intial_state, 1)
    );
    result.add_transition(
      result.intial_state,
      sigma,
    ...
    Object.keys(this.transition).forEach((f) => {
      Object.keys(this.transition[f]).forEach((via) => {
        if (this.transition[f][via].indexOf(from) != -1) {
          this.transition[f][via].splice(
            this.transition[f][via].indexOf(from),
            1
          );
  ...
  add_dead_state() {
    ...
    this.inputs.forEach((via) => {
      this.states.forEach((from) => {
        if (
          this.transition[from] == undefined ||
          this.transition[from][via] == undefined
        ) {
